...
RUN exec /usr/bin/mongod & sleep 10; mongo learninglocker --eval --smallfiles 'printjson(db.createUser({user:"username",pwd:"password",roles:["readWrite"]}))'; cd /learninglocker; php artisan migrate
